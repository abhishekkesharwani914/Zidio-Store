  ...
  switch (event.type) {
    case "checkout.session.completed":
      ...
      if (session.payment_status === "paid") {
        ...
        console.log(
          `Checkout session completed, but payment status is ${session.payment_status}: ${session.id}`
        );
    ...
    case "payment_intent.payment_failed":
      ...
      console.log(
        `PaymentIntent failed: ${failedPaymentIntent.id}, Reason: ${failedPaymentIntent.last_payment_error?.message}`
      );
